...
- (BOOL)addPerson:(NSString *)personName andPhoneNum:(NSString *)num
{
    ABAddressBookRef addressBook = ABAddressBookCreate();
    CFErrorRef anError = NULL;
    ABRecordRef aContact = ABPersonCreate();
    CFStringRef cfName = CFStringCreateWithCString(kCFAllocatorDefault, [personName UTF8String], kCFStringEncodingUTF8);
    ABRecordSetValue(aContact, kABPersonFirstNameProperty, cfName, &anError);
    ABMutableMultiValueRef phone = ABMultiValueCreateMutable(kABMultiStringPropertyType);
    ABMultiValueAddValueAndLabel(phone, num, kABPersonPhoneMobileLabel, NULL);
    ...
    ABAddressBookSave(addressBook, nil);//保存
    return YES;
}
#pragma  mark 添加联系人
...
- (BOOL)addContactName:(NSString*)name phoneNum:(NSString*)num withLabel:(NSString*)label{
    ...
    ABMutableMultiValueRef phone = ABMultiValueCreateMutable(kABMultiStringPropertyType);
    ABMultiValueAddValueAndLabel(phone, num, kABPersonPhoneIPhoneLabel, NULL);
